...
public class UserController implements CommunityConstant {
    ...
    public String updatePassword(String oldPassword, String newPassword, Model model) {
        User user = hostHolder.getUser();
        String oldPa = CommunityUtil.md5(oldPassword + user.getSalt());
        if (!oldPa.equals(user.getPassword())) {
            model.addAttribute("oldPasswordMsg", "密码错误!");
            return "/site/setting";
        }

        if (newPassword.length() > 8) {
            model.addAttribute("newPasswordMsg", "密码长度不能大于8位!");
            return "/site/setting";
        }
        String newPa = CommunityUtil.md5(newPassword + user.getSalt());
        userService.updatePassword(user.getId(), newPa);
        hostHolder.clear();
